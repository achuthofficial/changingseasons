// ISO 3166-1 alpha-2 code, name, and calling code — used by PhoneInput to
// let a customer's phone number be tagged with the country it's from,
// since the boutique isn't only serving India-based customers.
...
// Two-letter region codes map to Unicode "regional indicator" letters,
// which most platforms render as a flag emoji automatically — no image
// assets needed.
...
// Longest-dial-code-first so e.g. "+1868" (Trinidad) matches before the
// shorter "+1" (US/Canada) when parsing a stored phone number back apart.
const byDialLengthDesc = [...countries].sort((a, b) => b.dial.length - a.dial.length)

export function parsePhone(value) {
  const trimmed = (value ?? '').trim()
  if (trimmed.startsWith('+')) {
    const match = byDialLengthDesc.find((c) => trimmed.startsWith(c.dial))
    if (match) {
      return { country: match, number: trimmed.slice(match.dial.length).trim() }
    }
  }
  return { country: countries[0], number: trimmed }
}
